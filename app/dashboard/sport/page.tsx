...
// Sekin va silliq chiqish variantlari
const fadeInUp = {
...
const staggerContainer = {
  initial: {},
  whileInView: { transition: { staggerChildren: 0.6 } }
};
...
export default function Sport() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 overflow-x-hidden">
      {/* 1. HEADER SECTION */}
      <div className="relative border-b border-amber-900/10 bg-[#0d0f12] py-5 overflow-hidden group">
        <motion.div 
          initial={{ opacity: 0, x: -30 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 1 }}
          className="max-w-7xl mx-auto px-6 relative z-10 flex items-center justify-between"
        >
          <div className="flex items-center gap-4">
            <h1 className="text-2xl md:text-3xl font-black italic tracking-tighter text-white uppercase -skew-x-6">
              Sport
            </h1>
            <motion.div 
              initial={{ width: 0 }}
              ...
              className="h-[2px] bg-amber-500 rounded-full"
            ></motion.div>
            <p className="hidden md:block text-slate-500 text-xs uppercase tracking-widest font-medium italic">
              Power & Performance
            </p>
          </div>
          <span className="text-[10px] text-amber-500/50 uppercase font-bold tracking-widest">Tez mashinalar</span>
        </motion.div>
      </div>
