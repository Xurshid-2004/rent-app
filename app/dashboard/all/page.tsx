...
export default function All() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 overflow-x-hidden">
      ...
      {/* --- 1. HEADER SECTION ({t.landing.allCars}) --- */}
      <motion.div 
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
        variants={fadeIn}
        className="relative border-b border-amber-900/20 bg-[#0a0b0d] py-6 overflow-hidden group"
      >
        <div className="absolute inset-0 bg-gradient-to-r from-amber-500/0 via-amber-500/5 to-transparent pointer-events-none"></div>
        <div className="max-w-7xl mx-auto px-6 relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <h1 className="text-2xl md:text-3xl font-serif italic text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-100">
              {t.landing.allCars}
            </h1>
            <div className="h-[1px] w-12 bg-amber-500/50 hidden md:block"></div>
            <p className="text-slate-500 text-[10px] md:text-xs uppercase tracking-[0.2em] font-medium">
              {t.landing.exclusiveCollection}
            </p>
          </div>
      ...
      {/* --- 2. CARS GRID SECTION --- */}
      <div id="card" className="py-6 sm:py-12 relative overflow-hidden bg-gradient-to-br from-gray-900 via-black to-gray-900 ">
        <div className="absolute top-0 left-0 w-48 h-48 sm:w-96 sm:h-96  bg-blue-500/10 blur-3xl rounded-full"></div>
        <div className="absolute bottom-0 right-0 w-[48] h-[48] sm:w-96 sm:h-96 bg-purple-500/10 blur-3xl rounded-full"></div>
          <CarGrid category="all" />
      </div>
